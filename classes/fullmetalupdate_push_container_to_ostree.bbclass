...
do_push_container_to_ostree_and_hawkbit() {
    ...
    json=$(curl_post "/" '[ { "vendor" : "'${HAWKBIT_VENDOR_NAME}'", "name" : "'${OSTREE_PACKAGE_BRANCHNAME}'", "description" : "'$OSTREE_PACKAGE_BRANCHNAME'-'${MACHINE}'", "type" : "application", "version" : "'$(date +%Y%m%d%H%M)'" } ]')
    prop='id'
